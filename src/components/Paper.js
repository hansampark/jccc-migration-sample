...
const shadow = (zDepth, rgb = [0, 0, 0]) => {
  const colorRGB = rgb.join(',');

  return [
    `${ELEVATION_UMBRA_MAP[zDepth]} rgba(${colorRGB}, ${UMBRA_OPACITY})`,
    `${ELEVATION_PENUMBRA_MAP[zDepth]} rgba(${colorRGB}, ${PENUMBRA_OPACITY})`,
    `${ELEVATION_AMBIENT_MAP[zDepth]} rgba(${colorRGB}, ${AMBIENT_OPACITY})`,
  ].join(',');
};
...
const Paper = styled.div`
  ...
  box-shadow: ${(props) => shadow(props.zDepth)};
